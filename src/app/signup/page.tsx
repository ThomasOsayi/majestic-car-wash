...
/* ===== DATA ===== */
const PLANS: Record<string, { name: string; monthly: number; annual: number; desc: string }> = {
  essential: { name: "Essential", monthly: 34.99, annual: 349.99, desc: "Unlimited exterior hand wash" },
  premium: { name: "Premium", monthly: 49.99, annual: 499.99, desc: "Unlimited full-service hand wash" },
  ultimate: { name: "Ultimate", monthly: 64.99, annual: 649.99, desc: "Full-service + monthly detail" },
};
...
/* ===== PAYMENT FORM ===== */
function PaymentForm({ onSuccess, onBack, planName, isAnnual, dueToday, summaryLines }: {
  onSuccess: () => void; onBack: () => void; planName: string;
  isAnnual: boolean; dueToday: number; summaryLines: { label: string; value: string; bold?: boolean; savings?: boolean }[];
}) {
  ...
  async function handlePay() {
    ...
    if (error) { setPayError(error.message || "Payment failed. Please try again."); setProcessing(false); }
    else { onSuccess(); }
  }
  ...
  return (
    <div className="signup-step-content">
      ...
      <p className="signup-desc">Complete your {planName} {isAnnual ? "annual" : "monthly"} membership payment securely.</p>

      <div className="summary-box" style={{ marginBottom: "24px" }}>
        {summaryLines.map((line, i) => (
          line.savings ? (
            <div className="summary-savings" key={i}><span>{line.label}</span></div>
          ) : line.label === "---" ? (
            <hr className="summary-divider" key={i} />
          ) : (
            <div className={`summary-row${line.bold ? " summary-total-row" : ""}`} key={i}>
      ...
      <div className="signup-btns" style={{ marginTop: "24px" }}>
        ...
        <button className="signup-btn gold" onClick={handlePay} disabled={!stripe || processing} style={processing ? { opacity: 0.6, cursor: "not-allowed" } : {}}>
          {processing ? "Processing payment..." : `Pay $${dueToday.toFixed(2)} — Start Membership`}
        </button>
      </div>
      <div className="secure-note"><span>🔒</span><span>Secure payment powered by Stripe.</span></div>
    </div>
  );
}
...
function SignupForm() {
  ...
  const [billingInterval, setBillingInterval] = useState<"monthly" | "annual">(intervalParam === "annual" ? "annual" : "monthly");
  const [vehicleType, setVehicleType] = useState("sedan");
  const [surcharge, setSurcharge] = useState(0);
  const [form, setForm] = useState({ make: "", model: "", color: "", plate: "", firstName: "", lastName: "", email: "", phone: "", password: "", confirmPassword: "" });
  ...
  const monthlyBase = currentPlan.monthly + surcharge;
  const annualBase = currentPlan.annual + (surcharge * 12);
  const annualPerMonth = annualBase / 12;
  const annualSavings = (currentPlan.monthly * 12) - currentPlan.annual;
  const dueToday = isAnnual ? annualBase : PROMO_PRICE;
  const displayPrice = isAnnual ? currentPlan.annual : currentPlan.monthly;
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) { if (searchWrapRef.current && !searchWrapRef.current.contains(e.target as Node)) setMakeDropdownOpen(false); }
    document.addEventListener("click", handleClick);
    return () => document.removeEventListener("click", handleClick);
  }, []);

  function updateField(field: string, value: string) { setForm(prev => ({ ...prev, [field]: value })); if (field === "password" || field === "confirmPassword") setPasswordError(""); }
  ...
  // Build summary lines for payment step
  function getSummaryLines() {
    const lines: { label: string; value: string; bold?: boolean; savings?: boolean }[] = [];
    if (isAnnual) {
      lines.push({ label: `${currentPlan.name} Plan (Annual)`, value: `$${currentPlan.annual.toFixed(2)}/yr` });
      if (surcharge > 0) lines.push({ label: "SUV/Van surcharge (12 mo)", value: `+$${(surcharge * 12).toFixed(2)}/yr` });
      lines.push({ label: "---", value: "" });
      lines.push({ label: "Due today", value: `$${annualBase.toFixed(2)}`, bold: true });
      lines.push({ label: `✓ Annual billing — save $${annualSavings.toFixed(2)} vs monthly`, value: "", savings: true });
      lines.push({ label: "Effective monthly rate", value: `$${annualPerMonth.toFixed(2)}/mo` });
      lines.push({ label: "Next charge", value: getNextChargeDate() });
    } else {
      lines.push({ label: `${currentPlan.name} Plan`, value: `$${currentPlan.monthly.toFixed(2)}/mo` });
      if (surcharge > 0) lines.push({ label: "SUV/Van surcharge", value: `+$${surcharge.toFixed(2)}/mo` });
      lines.push({ label: "---", value: "" });
      lines.push({ label: "Monthly after trial", value: `$${monthlyBase.toFixed(2)}/mo` });
      lines.push({ label: "Due today", value: `$${PROMO_PRICE.toFixed(2)}`, bold: true });
      lines.push({ label: `✓ First month promo — you save $${(monthlyBase - PROMO_PRICE).toFixed(2)}`, value: "", savings: true });
      lines.push({ label: "Next charge", value: getNextChargeDate() });
    }
    return lines;
  }
  ...
  async function validateAndGoToPayment() {
    ...
    setPasswordError(""); setLoadingPayment(true);

    try {
      const res = await fetch("/api/create-subscription", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ plan, interval: billingInterval, firstName: form.firstName, lastName: form.lastName, email: form.email, phone: form.phone, vehicleType, make: form.make, model: form.model, color: form.color, plate: form.plate, surcharge }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to set up payment");
      setClientSecret(data.clientSecret); setStripeCustomerId(data.customerId); setStripeSubscriptionId(data.subscriptionId);
      setStep(4);
    } catch (error: any) { console.error("Payment setup failed:", error); alert(error.message || "Something went wrong."); }
  ...
  async function handlePaymentSuccess() {
    ...
    try {
      let authUid = "";
      try { const cred = await createUserWithEmailAndPassword(auth, form.email, form.password); authUid = cred.user.uid; }
      catch (authErr: any) { if (authErr.code !== "auth/email-already-in-use") console.error("Auth creation failed:", authErr); }

      const memberSince = new Date().toLocaleDateString("en-US", { month: "short", year: "numeric" });
      const memberId = await createMember({
        firstName: form.firstName, lastName: form.lastName, email: form.email, phone: form.phone,
        plan: plan as "essential" | "premium" | "ultimate", planName: currentPlan.name,
        price: isAnnual ? currentPlan.annual : currentPlan.monthly,
        status: "active",
        vehicleType: vehicleType as "sedan" | "suv" | "van",
        make: form.make, model: form.model, color: form.color, plate: form.plate.toUpperCase(),
        surcharge, memberSince, nextBilling: getNextChargeDate(),
        stripeCustomerId, stripeSubscriptionId, authUid,
        billingInterval: billingInterval,
      });
      localStorage.setItem("memberId", memberId);
      setStep(5);
    } catch (err: any) { console.error("Account creation failed:", err); setStep(5); }
  ...
  const elementsOptions = clientSecret ? {
    ...
    appearance: {
      ...
      variables: { colorPrimary: "#D4202C", colorBackground: "#1A1714", colorText: "#FFFFFF", colorDanger: "#FF2D3B", borderRadius: "12px", fontFamily: "Outfit, sans-serif" },
      rules: {
    ...
    <div className="signup-page">
      ...
      <div className="signup-container">
        ...
        <div className="signup-card">
          ...
          {/* STEP 1: Plan + Billing Toggle */}
          {step === 1 && !hasPreselectedPlan && (
            <div className="signup-step-content">
              <h2 className="signup-title">Choose Your Plan</h2>
              <p className="signup-desc">Unlimited hand washes. Cancel anytime.</p>

              <div className="billing-toggle-wrap" style={{ marginBottom: "24px" }}>
                <div className="billing-toggle billing-toggle-dark">
                  <button className={`billing-toggle-btn${!isAnnual ? " active" : ""}`} onClick={() => setBillingInterval("monthly")}>Monthly</button>
                  <button className={`billing-toggle-btn${isAnnual ? " active" : ""}`} onClick={() => setBillingInterval("annual")}>Annual <span className="billing-save-badge">Save 15%</span></button>
                </div>
              </div>
                ...
                {Object.entries(PLANS).map(([key, p]) => (
                  <div key={key} className={`plan-option${plan === key ? " selected" : ""}`} onClick={() => setPlan(key)}>
                    ...
                    <div className="plan-price-col">
                      {isAnnual ? (
                        <>
                          <div className="plan-price-main">${p.annual.toFixed(2)}</div>
                          <div className="plan-price-period">per year</div>
                          <div className="plan-price-eff">${(p.annual / 12).toFixed(2)}/mo</div>
                        </>
                      ) : (
                        <>
                          <div className="plan-price-main">${p.monthly.toFixed(2)}</div>
              ...
              <div className="promo-note-box">
                {isAnnual ? (
                  <><span>💰</span><p>Annual billing — <strong>save up to $129/yr</strong>. Full year paid upfront.</p></>
                ) : (
                  <><span>🎉</span><p>First month just <strong>$14.99</strong> on any plan. New members only.</p></>
                )}
              </div>
            ...
            <div className="signup-step-content">
              ...
              <div className="vehicle-types">
                {[
                  ...
                  { type: "suv", icon: "🚙", name: "SUV / Truck", extra: 5, priceLabel: isAnnual ? "+$60.00/yr" : "+$5.00/mo" },
                  { type: "van", icon: "🚐", name: "Minivan / Van", extra: 5, priceLabel: isAnnual ? "+$60.00/yr" : "+$5.00/mo" },
                ].map(v => (
              ...
              {selectedMakeObj && <div className="make-badge"><div className="make-badge-logo">{selectedMakeObj.icon}</div><div className="make-badge-info"><div className="make-badge-name">{selectedMakeObj.name}</div><div className="make-badge-sub">Pick your model below</div></div><button className="make-badge-clear" onClick={clearMake}>✕</button></div>}
              {!selectedMakeObj && (
                <div className="make-search-wrap" ref={searchWrapRef}>
                  <div className="make-search-icon">🔍</div>
                  <input type="text" className="make-search-input" placeholder="Type your car brand..." value={makeQuery} onChange={e => { setMakeQuery(e.target.value); setMakeDropdownOpen(true); }} onFocus={() => setMakeDropdownOpen(true)} />
                  {makeDropdownOpen && (
                    <div className="make-dropdown">
                      {filteredMakes.length === 0 ? <div className="make-dropdown-empty">No match found.</div> : <>
                        ...
                        {(showPopularHeader ? popularMakes : filteredMakes).map(m => <div key={m.name} className="make-dropdown-item" onClick={() => selectMake(m)}><div className="make-dropdown-item-logo">{m.icon}</div><div className="make-dropdown-item-text"><div className="make-dropdown-item-name">{m.name}</div><div className="make-dropdown-item-meta">{m.models.slice(0,3).join(", ")}...</div></div></div>)}
                        {showPopularHeader && otherMakes.length > 0 && <><div className="make-dropdown-section">All Brands</div>{otherMakes.map(m => <div key={m.name} className="make-dropdown-item" onClick={() => selectMake(m)}><div className="make-dropdown-item-logo">{m.icon}</div><div className="make-dropdown-item-text"><div className="make-dropdown-item-name">{m.name}</div><div className="make-dropdown-item-meta">{m.models.slice(0,3).join(", ")}...</div></div></div>)}</>}
                      </>}
                    </div>
                  )}
                </div>
              )}
              {selectedMakeObj && <div className="model-chips-section"><div className="model-chips">{selectedMakeObj.models.map(m => <div key={m} className={`model-chip${form.model === m ? " selected" : ""}`} onClick={() => selectModel(m)}>{m}</div>)}</div><div className="model-other-row"><span className="model-other-label">or type:</span><input type="text" className="model-other-input" placeholder="Other model..." value={selectedMakeObj.models.includes(form.model) ? "" : form.model} onChange={e => updateField("model", e.target.value)} onFocus={() => { if (selectedMakeObj.models.includes(form.model)) updateField("model", ""); }} /></div></div>}
          ...
          {step === 3 && (
            <div className="signup-step-content">
              ...
              <div className="field-group"><label className="signup-label">Email Address</label><input className="signup-input" type="email" placeholder="john@example.com" value={form.email} onChange={e => updateField("email", e.target.value)} autoComplete="off" /></div>
              <div className="field-group"><label className="signup-label">Phone Number</label><input className="signup-input" type="tel" placeholder="(310) 555-1234" value={form.phone} onChange={e => updateField("phone", e.target.value)} autoComplete="off" /></div>
              <div style={{ marginTop: "8px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                <div className="field-row">
                  <div className="field-group"><label className="signup-label">Password</label><input className="signup-input" type="password" placeholder="At least 6 characters" value={form.password} onChange={e => updateField("password", e.target.value)} autoComplete="new-password" /></div>
                  <div className="field-group"><label className="signup-label">Confirm Password</label><input className="signup-input" type="password" placeholder="Confirm password" value={form.confirmPassword} onChange={e => updateField("confirmPassword", e.target.value)} autoComplete="new-password" /></div>
                </div>
              ...
              <div className="signup-btns">
                ...
                <button className="signup-btn primary" onClick={validateAndGoToPayment} disabled={loadingPayment} style={loadingPayment ? { opacity: 0.6 } : {}}>{loadingPayment ? "Setting up payment..." : "Continue to Payment →"}</button>
              </div>
            </div>
          )}
          ...
          {step === 4 && clientSecret && elementsOptions && (
            <Elements stripe={stripePromise} options={elementsOptions}>
              <PaymentForm onSuccess={handlePaymentSuccess} onBack={() => setStep(3)} planName={currentPlan.name} isAnnual={isAnnual} dueToday={dueToday} summaryLines={getSummaryLines()} />
            </Elements>
          )}
              ...
              {creatingAccount ? (
                ...
                <>
                  ...
                  <div className="confirm-details">
                    ...
                    <div className="confirm-row"><span>Plan</span><strong>{currentPlan.name} ({isAnnual ? "Annual" : "Monthly"})</strong></div>
                    ...
                    <div className="confirm-row"><span>Charged today</span><strong className="gold-text">${dueToday.toFixed(2)}</strong></div>
                    <div className="confirm-row"><span>Next charge</span><strong>{getNextChargeDate()}</strong></div>
                    {isAnnual && <div className="confirm-row"><span>Annual savings</span><strong className="gold-text">${annualSavings.toFixed(2)}/yr</strong></div>}
                  </div>
